...
public class Simulation {
	...
	private static int batheryToSend;
	private static int batheryToReceive;

	public static int getBatheryToSend() {
		return batheryToSend;
	}

	public static void setBatheryToSend(int batheryToSend) {
		Simulation.batheryToSend = batheryToSend;
	}

	public static int getBatheryToReceive() {
		return batheryToReceive;
	}

	public static void setBatheryToReceive(int batheryToReceive) {
		Simulation.batheryToReceive = batheryToReceive;
	}
	...
	public static void packageTransference(ArrayList<Vertex> vertex, int[] houteringTree) {
		for (int k = 1; k < vertex.size(); k++) {
			...
			if (vertex.get(k).getPackages().size() != 0 && vertex.get(k).getBathery() > batheryToSend && vertex.get(k).isActivate())
	...
	private static void sendToSink(Vertex v1, Vertex v2) {
		while (true) {
			if (v1.getPackages().size() == 0 || v1.getBathery() <= batheryToSend)
				break;
			else if (Probability.packageLost()) {
				v1.getPackages().poll();
				v1.setBathery(batheryToSend);
			} else {
	...
	private static void sendToVertex(Vertex v1, Vertex v2) {
		while (true) {
			...
			if (v1.getPackages().size() == 0 || v1.getBathery() <= batheryToSend || v2.getBathery() <= batheryToReceive)
				break;
			//se o pacote for perdido o vertice de destino mantem a bateria e a quantidade de pacotes atual
				...
				v2.getPackages().add(v1.getPackages().poll());
				v1.setBathery(batheryToSend);
				v2.setBathery(batheryToReceive);
			}
		}
	}
}
